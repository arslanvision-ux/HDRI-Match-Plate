...
p = next(p for p in parts if p['name'] == 'C') # Let's test on the Beauty pass!
...
def try_unpredict_1(raw):
    # The original implementation in exr_pure.py:
    n = len(raw)
    half = (n + 1) // 2
...
def try_unpredict_2(raw):
    # Uninterleave first, then unpredict
    n = len(raw)
    half = (n + 1) // 2
    ...
    t = uninterleaved.astype(np.int32)
    t[1:] -= 128
    np.cumsum(t, out=t)
    return t.astype(np.uint8).tobytes()

def extract(scan_data, scan_y):
...
for off in offsets:
    ...
    res1 = extract(try_unpredict_1(raw), scan_y)
    print("Method 1 (original): min", res1.min(), "max", res1.max())
    
    res2 = extract(try_unpredict_2(raw), scan_y)
    print("Method 2 (uninterleave first): min", res2.min(), "max", res2.max())
    break
