...
def extract_channel_then_scanline(scan_data, scan_y):
...
def extract_scanline_then_channel(scan_data, scan_y):
    result = np.zeros((h, w, len(channels)), dtype=np.float32)
    lines_in_block = min(16, h - scan_y)
    sc_off = 0
    for dy in range(lines_in_block):
...
for off in offsets:
    ...
    res1 = extract_channel_then_scanline(scan_data, scan_y)
    print("Channel then Scanline: min", res1.min(), "max", res1.max())
    
    res2 = extract_scanline_then_channel(scan_data, scan_y)
    print("Scanline then Channel: min", res2.min(), "max", res2.max())
    break
